...
export function RewardCard({ reward, compact = false }: RewardCardProps) {
  ...
  // Format tailor name
  const getTailorName = (tailor?: { firstName: string | null; lastName: string | null }) => {
    if (!tailor) return 'Unknown Tailor';
    return `${tailor.firstName || ''} ${tailor.lastName || ''}`.trim() || 'Anonymous Tailor';
  };
    ...
    return (
      <div className="bg-gray-800/40 backdrop-blur-sm border border-gray-800 rounded-lg p-4 hover:bg-gray-800/60 transition-colors">
        <div className="flex justify-between items-start">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">{getTailorName(reward.tailor)}</p>
            </div>
          </div>
          <Badge 
            className={
              reward.type === 'DISCOUNT' ? 'bg-green-600' :
              reward.type === 'FREE_ITEM' ? 'bg-purple-600' :
              reward.type === 'POINTS' ? 'bg-blue-600' :
              'bg-amber-600'
            }
          >
            {reward.type === 'DISCOUNT' ? `${reward.value}% OFF` :
             reward.type === 'FREE_ITEM' ? `${reward.value} FREE` :
        ...
        <div className="mt-2 flex justify-between items-center">
          ...
          >
            {copiedCode ? <CheckCircle className="h-3 w-3 mr-1 text-green-500" /> : <Clipboard className="h-3 w-3 mr-1" />}
            {copiedCode ? 'Copied' : 'Copy Code'}
          </Button>
          
          <p className="text-xs text-gray-500">Expires {formatDate(reward.endDate)}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm mb-4">
          ...
          <div>
            ...
            <p className="font-medium truncate">{getTailorName(reward.tailor)}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={redeemMutation.isPending && redeemMutation.variables?.rewardId === reward.id}
          >
            {redeemMutation.isPending && redeemMutation.variables?.rewardId === reward.id ? (
              <>
