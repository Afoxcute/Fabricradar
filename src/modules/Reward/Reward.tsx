...
function TailorRewardsPage() {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<
    'active' | 'draft' | 'expired' | 'create' | 'tokens'
  >('active');
  ...
  const filteredRewards = rewards.filter((reward: Reward) => {
    if (activeTab === 'active')
      return reward.isActive && new Date(reward.endDate) >= new Date();
      ...
      <div className="flex justify-between items-start mb-8 flex-wrap">
        <div className="w-full max-w-[400px] gap-3">
          ...
          <p className="text-gray-400">
            Create and manage special offers and rewards for your customers
          </p>
        </div>
        {activeTab !== 'tokens' ? (
          <Button
          ...
          <div className="flex gap-4">
            ...
            <Button
              onClick={() =>
                document
                  .getElementById('createTokenSection')
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gray-900/60 backdrop-blur-sm border border-gray-800 rounded-xl p-6 flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {rewards.filter((r: Reward) => r.isActive).length}
              </p>
            </div>
          </div>
          ...
          <div className="bg-gray-900/60 backdrop-blur-sm border border-gray-800 rounded-xl p-6 flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {rewards.reduce(
                  (sum: number, r: Reward) => sum + r.redemptionCount,
                  0
                )}
              </p>
            </div>
          ...
          {filteredRewards.length === 0 && (
            <div className="bg-gray-900/60 backdrop-blur-sm border border-gray-800 rounded-xl p-8 text-center">
              ...
              <p className="text-gray-400 mb-6">
                {activeTab === 'active'
                  ? "You don't have any active rewards. Create one to engage your customers."
                  : activeTab === 'expired'
                    ? 'No expired rewards found.'
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <input
                        type="date"
                        value={
                          new Date(selectedReward.startDate)
                            .toISOString()
                            .split('T')[0]
                        }
                        onChange={(e) =>
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={
                          new Date(selectedReward.endDate)
                            .toISOString()
                            .split('T')[0]
                        }
                        onChange={(e) =>
...
export default TailorRewardsPage;
