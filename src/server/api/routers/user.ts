...
export const userRouter = createTRPCRouter({
  ...
  // Get all customers who have ordered from a specific tailor
  getTailorCustomers: publicProcedure
    .input(z.object({
      tailorId: z.number(),
    }))
    .query(async ({ ctx, input }) => {
      ...
      try {
        // Get all unique user IDs who have ordered from this tailor
        const orderUserIds = await ctx.db.order.findMany({
          where: {
        ...
        const customers = await ctx.db.user.findMany({
          where: {
            id: {
              in: orderUserIds.map(order => order.userId),
            },
            accountType: "USER", // Ensure they are customers
          },
          select: {
            id: true,
            firstName: true,
            ...
            phone: true,
            createdAt: true,
            // Explicitly exclude walletAddress for privacy
            // Include order count
            _count: {
              select: {
                orders: {
            ...
            // Get their orders with this tailor
            orders: {
              where: {
                tailorId: input.tailorId,
              },
        ...
        console.error('Error fetching tailor customers:', error);
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
